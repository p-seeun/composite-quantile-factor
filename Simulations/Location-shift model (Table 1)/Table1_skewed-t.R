...
for(n in 1:5){
  ...
  for (s in 1:rep) {
    ...
    e =  matrix(rst(Te * N, xi=0,omega=5,alpha=5,nu=3), nrow = Te, ncol = N)-5*sqrt(3/pi)*gamma(1)/gamma(1.5)*(5/sqrt(26))   
    
    X <- F %*% t(Lambda) + e
    
    weight=dst(qst(c(0.1,0.3,0.5,0.7,0.9), xi=0,omega=5,alpha=5,nu=3), xi=0,omega=5,alpha=5,nu=3)

    # Estimate factors
    Fhat_RobPCA = robpca_est(X, r=3)$fmat
    Fhat_Huang = cqrfactor(X, r=3, tau=c(0.1,0.3,0.5,0.7,0.9), tol=1e-4)$fmat
    Fhat_DAFM = dafm_est(X, r=4, tau.vec=c(0.1,0.3,0.5,0.7,0.9), tol=1e-4, weight=weight)$fmat
...
for(n in 1:5){
  ...
  for (s in 1:rep) {
    ...
    Fhat_QFM0.5 = qfm_est(X, 4, tol, tau=0.5)$fmat
...
Table1_skewed_t = matrix(nrow=8, ncol=0)
for(n in 1:5){
  table = rbind(RobPCA[[n]], QFM0.1[[n]], QFM0.3[[n]], QFM0.5[[n]], QFM0.7[[n]], QFM0.9[[n]], Huang[[n]], DAFM[[n]])
  Table1_skewed_t = cbind(Table1_skewed_t, table)
}

Table1_skewed_t # third table in Table 1
