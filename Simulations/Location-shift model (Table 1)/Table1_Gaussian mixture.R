#Location-shift model: Gaussian mixture
...
for(n in 1:5){
  ...
  for (s in 1:rep) {
    ...
    e=c()
    for(i in 1:(Te*N)){
      e[i] = ifelse(runif(1)>0.5, rnorm(1, 2, 0.5),rnorm(1, -2, 0.5))
    }
    e = matrix(e, nrow=Te, ncol=N)
    
    X <- F %*% t(Lambda) + e
    
    d <- density(e, kernel = "gaussian")
    m = round((quantile(e,c(0.1,0.3,0.5,0.7,0.9))-d$x[1])/(d$x[2]-d$x[1]))+1
    weight = d$y[m]/sum(d$y[m])
...
Table1_mixgauss = matrix(nrow=7, ncol=0)
for(n in 1:5){
  table = rbind(RobPCA[[n]], QFM0.1[[n]], QFM0.3[[n]], QFM0.5[[n]], QFM0.7[[n]], QFM0.9[[n]], Huang[[n]], DAFM[[n]])
  Table1_mixgauss = cbind(Table1_mixgauss, table)
}

Table1_mixgauss # second table in Table 1
